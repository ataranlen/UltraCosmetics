...
public class MountSnake extends Mount {
    ...
    private final HashMap<Player, ArrayList<Entity>> tailMap = new HashMap();
    private int color = 1;
    ...
    @Override
    public void clear() {
        tailMap.keySet().stream().forEach((Player p) -> {
            tailMap.get(p).stream().forEach((ent) -> {
                ent.remove();
            });
        });
        tailMap.clear();
    ...
    protected void onUpdate() {
        if (getPlayer() != null) {
            Bukkit.getScheduler().runTask(UltraCosmetics.getInstance(), () -> {
                if (getPlayer() != null) {
