...
public class MountEcologistHorse extends Mount {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        if (event.getPlayer() == getPlayer()
            ...
            datas.add((byte) 0x5);
            datas.add((byte) 0xd);
            BlockUtils.getBlocksInRadius(event.getPlayer().getLocation(), 3, false).stream().filter((b) -> (b.getLocation().getBlockY() == event.getPlayer().getLocation().getBlockY() - 1)).forEach((b) -> {
