...
public class UltraCosmetics extends JavaPlugin {
    ...
    public static void log(String object) {
        System.out.println("UltraCosmetics -> " + object);
    }
    ...
    public static boolean debug(Object message) {
        if (debug) {
            Bukkit.broadcastMessage("§c§lUC-DEBUG> §f" + message.toString());
        }
        return debug;
    }
    ...
    private void initPlayers() {
        Bukkit.getOnlinePlayers().stream().map((p) -> {
            ...
            return p;
        }).filter((Player p) -> ((List<String>) SettingsManager.getConfig().get("Enabled-Worlds")).contains(p.getWorld().getName())
                && (boolean) SettingsManager.getConfig().get("Menu-Item.Give-On-Join")).forEach((p) -> {
        ...
        if (!fileStorage) {
            Bukkit.getScheduler().runTaskTimerAsynchronously(this, () -> {
                try {
                    ...
                    PreparedStatement sql1 = co.prepareStatement("CREATE TABLE IF NOT EXISTS UltraCosmeticsData("
    ...
    private void setupCosmeticsConfigs() {
        ...
        // Emote
        GadgetType.values().stream().map((gadgetType) -> {
            ...
            return gadgetType;
        }).filter((gadgetType) -> (ammoEnabled)).map((gadgetType) -> {
        ...
        SuitType.values().stream().map((suit) -> {
        ...
        EmoteType.values().stream().map((emoteType) -> {
        ...
        EmoteType.values().stream().filter((emoteType) -> (emoteType.isEnabled())).forEach((emoteType) -> {
